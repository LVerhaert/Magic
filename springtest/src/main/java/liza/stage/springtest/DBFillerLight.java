...
public class DBFillerLight {
    ...
    CommandLineRunner init(MagicCardService magicCardService) throws IOException {
        ...
        List<MagicCardJson> magicCardJsonList = objectMapper.readValue(new File("D:\\lizav\\Documents\\Educatie\\Stage\\magic\\springtest\\src\\main\\resources\\scryfall-oracle-cards.json"), new TypeReference<List<MagicCardJson>>() {
        });
        return args -> {
            System.out.println("Saving..");
            magicCardJsonList.subList(0, 99).forEach(magicCardService::save);
//            System.out.println(magicCardJsonList.get(1));
            System.out.println("Saved.");
            System.out.println(magicCardService.findAll().size());
//            magicCardService.findAll().forEach(System.out::println);
//            System.out.println(magicCardService.findAll().get(1));
